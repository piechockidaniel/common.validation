...
public class ValidationResultTests
{
    ...
    {
        var r1 = new ValidationResult(new[] { new ValidationFailure("A", "err1") });
        var r2 = new ValidationResult(new[] { new ValidationFailure("B", "err2") });

        var merged = r1.Merge(r2);

        Assert.Equal(2, merged.Errors.Count);
        Assert.Contains(merged.Errors, e => e.PropertyName == "A");
        Assert.Contains(merged.Errors, e => e.PropertyName == "B");
    }
    ...
    public void Combine_CombinesMultipleResults()
    {
        var r1 = new ValidationResult(new[] { new ValidationFailure("A", "err1") });
        var r2 = new ValidationResult(new[] { new ValidationFailure("B", "err2") });
        var r3 = new ValidationResult(new[] { new ValidationFailure("C", "err3") });
    ...
    public void ToString_Invalid_ContainsErrorCount()
    {
        ...
        Assert.Contains("2 error(s)", result.ToString());
    }
}
